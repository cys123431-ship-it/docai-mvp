...
const DOCUMENT_ALIAS_MAP = {
  resume: ["이력", "cv", "자소서", "취업"],
  "jeonse-return": ["내용증명", "최고장", "통지서", "법적통지"],
  "standard-labor-contract": ["근로계약", "고용계약", "노무", "알바계약", "근무계약"],
  resignation: ["퇴직", "퇴사", "그만두기", "퇴직서"],
  "promissory-note": ["차용", "대여금", "차용금", "빌린돈", "돈빌림"],
  "freelance-service-contract": ["동업계약", "파트너십", "공동사업", "지분계약"],
  "employment-certificate": ["진정서", "민원", "노동청진정", "청원"],
  "career-certificate": ["경정청구", "세무정정", "세금환급", "환급청구"],
  "severance-request": ["보험금청구", "사고청구", "의료보험청구", "보험청구서"],
  "wage-arrears-notice": ["개명", "개명허가", "가정법원", "개명신청"],
  "lease-termination-notice": ["주택임대차", "주거임대차", "전월세계약", "임대차계약"],
  "rent-arrears-demand": ["상가임대차", "상가임대", "점포임대", "상가계약"],
  "deposit-return-confirmation": ["채무증명위임", "채무증명서", "발급위임", "대리발급"],
  "settlement-agreement": ["합의", "합의문", "분쟁합의", "화해"],
  "debt-repayment-commitment": ["친권자동의", "보호자동의", "미성년근로동의", "미성년자동의서"],
  "payment-order-application": ["지급명령", "법원신청", "독촉절차", "대여금청구"],
  "power-of-attorney": ["위임", "대리", "등기위임", "등기신청"],
  "statement-of-fact": ["혼인신고", "혼인등록", "결혼신고", "가족관계등록"],
  "personal-info-consent": ["사업계획서", "창업계획", "시장전략", "재무계획"],
...
const matchesDocument = (document, tokens) => {
  ...
  const searchableRaw = [document.title, document.subtitle, ...keywords, ...aliases].join(" ");
